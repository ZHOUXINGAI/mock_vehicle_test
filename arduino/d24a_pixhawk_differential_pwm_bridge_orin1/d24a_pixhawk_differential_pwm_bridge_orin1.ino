// Orin1 Pixhawk 6C differential rover PWM -> Arduino -> D24A -> four motors.
//
...
// Measured neutral input is 1488..1495 us. A 35 us deadband rejects that
// noise while allowing bounded low-speed Offboard commands to reach D24A.
const int DEAD_BAND_US = 35;
const int MIN_VALID_US = 900;
const int MAX_VALID_US = 2100;
// PX4 currently limits MAIN1/MAIN2 to 1300-1700us for the rover output range.
// Treat +/-200us as full stick so manual and offboard commands use the intended
// motor PWM range instead of topping out around analogWrite ~= 100.
const int MAX_STICK_US = 200;
const unsigned long PULSE_TIMEOUT_US = 25000;
const uint8_t VALID_PAIR_COUNT_REQUIRED = 3;

const int MIN_DRIVE_PWM = 70;
const int MAX_DRIVE_PWM = 255;

const bool SWAP_LEFT_RIGHT_INPUTS = false;
const bool INVERT_LEFT_COMMAND = true;
const bool INVERT_RIGHT_COMMAND = true;

unsigned long lastPrintMs = 0;
uint8_t validPairCount = 0;
...
void setup() {
  ...
  stopAll();
  Serial.begin(115200);
  Serial.println("D24A Orin1 differential PWM bridge ready deadband_us=35");
  delay(1000);
}
...
void loop() {
  ...
  bool validPair = input1Valid && input2Valid;
  if (!validPair) {
    validPairCount = 0;
    stopAll();
    leftCommand = 0;
    ...
    if (validPairCount < VALID_PAIR_COUNT_REQUIRED) {
      validPairCount++;
    }
    if (validPairCount < VALID_PAIR_COUNT_REQUIRED) {
      stopAll();
  ...
  if (now - lastPrintMs > 250) {
    ...
    Serial.print(" valid_pair_count=");
    Serial.println(validPairCount);
  }
