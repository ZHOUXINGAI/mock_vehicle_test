#!/usr/bin/env python3
"""Drive one raw D153B output channel for wiring calibration."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Raw D153B A/B channel calibration through Arduino serial."
    )
    parser.add_argument(
        "raw",
        choices=sorted(RAW_COMMANDS),
        help="raw driver channel/direction, before physical left/right calibration",
    )
...
def main() -> int:
    ...
    letter = RAW_COMMANDS[args.raw]

    print(f"Arduino port: {port}")
    print(f"Raw command: {args.raw} -> {letter} {pwm} {duration_ms}")
    print("Write down which physical motor moved and whether it was forward/backward.")
