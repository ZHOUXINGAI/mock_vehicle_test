#!/usr/bin/env python3
"""Run a wheels-up D24A four-wheel rover sequence through Arduino serial."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Forward/backward/left/right test via Arduino -> D24A."
    )
    parser.add_argument("--port", help="Arduino serial port; default auto-detect")
    parser.add_argument("--baud", type=int, default=115200)
    parser.add_argument("--pwm", type=int, default=140)
...
def main() -> int:
    ...
    print(f"Sequence: F, B, L, R; pwm={pwm}; duration={duration_ms}ms")
    print("Safety: wheels must be lifted; connect 12V only when ready.")
