#!/usr/bin/env python3
"""Drive one raw D24A output channel for four-wheel wiring calibration."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Raw D24A A/B/C/D channel calibration through Arduino serial."
    )
    parser.add_argument(
        "raw",
        choices=sorted(RAW_COMMANDS),
        help="raw driver channel/direction, before physical wheel calibration",
    )
    parser.add_argument("--port", help="Arduino serial port; default auto-detect")
    parser.add_argument("--baud", type=int, default=115200)
    parser.add_argument("--pwm", type=int, default=80)
...
def main() -> int:
    ...
    print(f"Raw command: {args.raw} -> {command} {pwm} {duration_ms}")
    print("Wheels must be lifted. Write down which wheel moved and direction.")

    if not args.armed:
        print("DRY RUN ONLY. Add --armed to drive the motor.")
        return 0
    ...
    try:
        ...
        line = f"{command} {pwm} {duration_ms}\n"
        print(f"send={line.strip()}")
        reply = write_line(fd, line)
