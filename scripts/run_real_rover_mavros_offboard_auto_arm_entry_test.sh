...
if [ "$OFFBOARD_LOG_DISABLE" != "true" ] && [ "${OFFBOARD_LOG_ACTIVE:-false}" != "true" ]; then
  ...
  echo "Saving Offboard auto-arm entry-test log:"
  ...
  echo "===== OFFBOARD AUTO-ARM ENTRY TEST LOG START $(date --iso-8601=seconds) ====="
  echo "cwd=$PWD"
  echo "command=$0 $*"
  echo
fi
...
cat <<'EOF'
MAVROS Offboard auto-arm entry test.

Default behavior:
  - wheels must be lifted
  - publishes only zero-velocity setpoints
  - requests OFFBOARD from the script
  - requests ARM from the script after OFFBOARD is accepted
  - disarms at the end
...
ARM_ON_START="true"
REQUIRE_ARMED_BEFORE_MODE_CHANGE="false"
...
cat <<EOF
Effective auto-arm entry-test settings:
