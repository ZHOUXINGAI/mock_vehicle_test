...
"""MAVROS Offboard out-and-back task for the real differential rover."""
...
class OutAndBackOffboard(Node):
    def __init__(self) -> None:
        super().__init__("real_rover_mavros_offboard_out_and_back")
        ...
        declare_param("stop_sec", 1.5)
        declare_param("final_stop_sec", 2.0)
        declare_param("forward_distance_m", 5.0)
        declare_param("linear_speed_mps", 0.25)
        declare_param("linear_direction_sign", 1.0)
        declare_param("turn_angle_deg", 180.0)
        declare_param("turn_yaw_rate_radps", 0.35)
        declare_param("turn_direction_sign", 1.0)
        declare_param("distance_tolerance_m", 0.15)
        declare_param("yaw_tolerance_deg", 8.0)
        declare_param("max_linear_speed_mps", 0.50)
        ...
        declare_param("require_armed_before_mode_change", True)
        ...
        declare_param("max_wait_for_ready_sec", 120.0)
        ...
        self.forward_distance_m = max(0.1, float(self.get_parameter("forward_distance_m").value))
        ...
        self.turn_angle_rad = math.radians(
            abs(float(self.get_parameter("turn_angle_deg").value))
        )
        ...
        self.get_logger().warn(
            "out-and-back task loaded: "
            f"distance={self.forward_distance_m:.2f}m "
            f"turn={math.degrees(self.turn_angle_rad):.1f}deg "
            f"speed={self.linear_speed_mps:.2f}m/s "
            f"yaw_rate={self.turn_yaw_rate_radps:.2f}rad/s"
        )

    def _validate(self) -> None:
        missing = []
        if self.test_surface != "ground":
            raise ValueError("out-and-back task requires test_surface='ground'")
        ...
        if missing:
            raise RuntimeError(
                "Refusing to run real out-and-back Offboard task until safety is confirmed: "
                + ", ".join(missing)
            )
    ...
    def _timer_cb(self) -> None:
        ...
        if self.stage == "initial_stop":
            self._publish_velocity(0.0, 0.0, 0.0)
            if self._stage_elapsed(now) >= self.stop_sec:
                self.leg_start_xy = self.current_xy
                self._enter_stage("leg1_forward_5m", now)
            return

        if self.stage == "leg1_forward_5m":
            self._publish_forward()
            distance = self._distance_from_leg_start()
            self._log_progress(now, f"leg1 distance={distance:.2f}m")
            if distance >= max(0.0, self.forward_distance_m - self.distance_tolerance_m):
        ...
        if self.stage == "stop_after_leg1":
            ...
            if self._stage_elapsed(now) >= self.stop_sec:
                ...
                self._enter_stage("turn_180", now)
            return

        if self.stage == "turn_180":
            self._publish_velocity(
                0.0,
                0.0,
                self.turn_yaw_rate_radps * self.turn_direction_sign,
            )
            yaw_delta = abs(self._yaw_delta_from_turn_start())
            self._log_progress(now, f"turn yaw_delta={math.degrees(yaw_delta):.1f}deg")
        ...
        if self.stage == "stop_after_turn":
            ...
            if self._stage_elapsed(now) >= self.stop_sec:
                self.leg_start_xy = self.current_xy
                self._enter_stage("leg2_forward_5m", now)
            return

        if self.stage == "leg2_forward_5m":
            self._publish_forward()
            distance = self._distance_from_leg_start()
            self._log_progress(now, f"leg2 distance={distance:.2f}m")
            if distance >= max(0.0, self.forward_distance_m - self.distance_tolerance_m):
        ...
        if self.stage == "final_stop":
            ...
            if self._stage_elapsed(now) >= self.final_stop_sec:
                ...
                self.get_logger().warn("out-and-back task complete; stop command sent")
                self.done = True
            return
    ...
    def _abort_sequence(self, reason: str) -> None:
        self._publish_velocity(0.0, 0.0, 0.0)
        self.get_logger().error(f"aborting out-and-back task: {reason}; stop sent")
        self.done = True
    ...
    def _publish_forward(self) -> None:
        self._publish_velocity(
            self.linear_speed_mps * self.linear_direction_sign,
            0.0,
            0.0,
        )

    def _publish_velocity(self, x_mps: float, y_mps: float, yaw_rate: float) -> None:
        msg = TwistStamped()
        ...
        if self.publish_unstamped_cmd_vel:
            ...
            unstamped.angular.z = float(yaw_rate)
    ...
    def _distance_from_leg_start(self) -> float:
    ...
    def _yaw_delta_from_turn_start(self) -> float:
        if self.turn_start_yaw is None or self.current_yaw is None:
            return 0.0
        return self._wrap_pi(self.current_yaw - self.turn_start_yaw)
