...
def mutated_plan_payload(offset: int, wire_format: str, value: int) -> bytes:
    payload = bytearray(sample_plan().encode())
    struct.pack_into(wire_format, payload, offset, value)
...
class CompactProtocolTest(unittest.TestCase):
    ...
    def test_frame_sizes_match_radio_budget(self):
        ...
        self.assertEqual(sizes["corridor_plan_payload"], 59)
...
class CommandGuardTest(unittest.TestCase):
    ...
    def ingest_plan_payload(self, payload: bytes):
        ...
        return gate.ingest(
            decoded_frame(MessageType.CORRIDOR_PLAN, payload), 10000
        )
    ...
    def test_plan_schema_and_timing_equations_are_fail_closed(self):
        faults = (
            (0, "<B", 3, "unsupported_plan_schema"),
            (47, "<H", 3349, "invalid_post_tangent_reserve"),
            (43, "<I", 29000, "invalid_required_validity"),
            (11, "<I", 30000, "plan_validity_below_required"),
        )
        for offset, wire_format, value, reason in faults:
            with self.subTest(reason=reason):
                result = self.ingest_plan_payload(
                    mutated_plan_payload(offset, wire_format, value)
                )
                self.assertEqual(result.decision, Decision.REJECT)
                self.assertEqual(result.reason, reason)

    def test_zero_plan_timings_are_rejected_except_guard(self):
        zero_faults = (
            (11, "<I", 1000, "invalid_plan_timing:validity_ms"),
            (27, "<I", 0, "invalid_plan_timing:mini_arrival_delay_ms"),
            (43, "<I", 0, "invalid_plan_timing:required_validity_ms"),
            (47, "<H", 0, "invalid_plan_timing:post_tangent_reserve_ms"),
            (49, "<H", 0, "invalid_plan_timing:terminal_completion_budget_ms"),
            (51, "<H", 0, "invalid_plan_timing:completion_hold_ms"),
            (55, "<H", 0, "invalid_plan_timing:command_ttl_ms"),
            (57, "<H", 0, "invalid_plan_timing:local_command_watchdog_ms"),
        )
        for offset, wire_format, value, reason in zero_faults:
            with self.subTest(reason=reason):
                result = self.ingest_plan_payload(
                    mutated_plan_payload(offset, wire_format, value)
                )
        ...
        payload = bytearray(sample_plan().encode())
        struct.pack_into("<H", payload, 53, 0)
        struct.pack_into("<H", payload, 47, 3250)
        struct.pack_into("<I", payload, 43, 29000)
        result = self.ingest_plan_payload(bytes(payload))
        self.assertEqual(result.decision, Decision.ACCEPT)
